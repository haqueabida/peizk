...
from interface import setup, enroll,revoke, updCred, prove, zkverify, createComs, proveAlgorithm, precomputeProofs
...
def timertest():
 ...
 #    #To add signature in helper and here
	...
	# enrolled=enrolled[:50]
	# credentials = credentials[:50]
	# time to create 10000 proofs and verify 10000 proofs
	fields = ["ID", "Commitment_Time", "Proof_Time", "C_a", "C_id", "C_B", "C_z", "C_e", "C_w", "pok2", "pok3", "pok4", "pok5", "pok6"]
	...
	for i in range(len(enrolled)):
		...
		preproofs = precomputeProofs(pp, stmt, proof1,  witness)
		...
		proof = proveAlgorithm(pp, stmt, proof1,  witness, preproofs)
		...
		with open("provetime.csv", 'a+', newline='') as csvfileP:
			...
			csvwriter.writerow([enrolled[i][0], com_time, proof_time, C_a, C_id, C_B, C_z, C_e, C_w, pok2, pok3, pok4, pok5, pok6])
	# proof_time = (time.time() - start_time)
	...
	# fields = ["ID", "verify_time"]
	# with open("verifytime.csv", 'w') as csvfileP:
	# 	csvwriter = csv.writer(csvfileP)
	# 	csvwriter.writerow(fields)
	...
	# 	verify_time = time.time() - start_time
	# 	# print((time.time() - start_time))
	# 	with open("verifytime.csv", 'a+', newline='') as csvfileP:
	# 		csvwriter = csv.writer(csvfileP)


	# 		csvwriter.writerow([enrolled[i][0], verify_time])
	...
	fields = ["ID", "Update_Time", "Commitment_Time", "Proof_Time"]
	with open("enrolled_update_prove50.csv", 'w') as csvfileU1:
	...
	for i in range(len(enrolled)):
		...
		# stmt, proof = prove(pp, setpk1, credentials[i], enrolled[i][0])
		proof_time = (time.time() - start_time)
		with open("enrolled_update_prove50.csv", 'a+', newline='') as csvfileU1:
			csvwriter = csv.writer(csvfileU1)
			csvwriter.writerow([enrolled[i][0], upd_time, com_time, proof_time])
