...
from helpfunctions import concat, generate_two_large_distinct_primes, hash_to_prime, bezoute_coefficients,\
    mul_inv, shamir_trick, calculate_product, hash_to_length
...
def nizkprove_exponentiation(g,h,u,x,w,n):
    
     B = n
     k = secrets.randbelow(B)
     p_x = secrets.randbelow(B)
     p_k = secrets.randbelow(B)
     
     z = (pow(g,x,n)*pow(h,p_x,n)) %n
     A_g = (pow(g,k,n)*pow(h,p_k,n)) %n
     A_u = pow(u,k,n)

     l = hash_to_prime(concat(u,w,z,A_g,A_u))
     c = hash_to_length(int(l[0]),ACCUMULATED_PRIME_SIZE)

     q_x = math.floor((k + c*x)//l[0])
     q_p = math.floor((p_k + c*p_x)//l[0])
     r_x = (k + c*x)%l[0]
     r_p = (p_k + c*p_x) % l[0]
     return l, z, (pow(g,q_x,n) * pow(h,q_p,n)) %n , pow(u,q_x,n), r_x, r_p



def verify_nizk(l,z,Q_g, Q_u, r_x, r_p, u, w,g, h, n):
    flag = True
    c = hash_to_length(int(l[0]),ACCUMULATED_PRIME_SIZE)
    A_g = ((pow(Q_g,l[0],n) * pow(g,r_x,n) * pow(h, r_p, n) * pow(mul_inv(z,n), c, n))%n ) %n
    A_u = ((pow(Q_u, l[0], n) * pow(u, r_x, n) * pow(mul_inv(w,n),c,n) )%n )%n


    if ((r_x > l[0]) or (r_x < -l[0])):
        flag = False

    if ((r_p > l[0]) or (r_p < -l[0])):
        flag = False


    if (l != hash_to_prime(concat(u, w, z ,A_g, A_u))):
        flag = False
